    ...
    private func createCalendar(_ call: FlutterMethodCall, _ result: FlutterResult) {
        ...
        do {
            ...
            if (calendarColor != nil) {
                calendar.cgColor = NSColor(hex: calendarColor!)?.cgColor
            }
            else {
                calendar.cgColor = NSColor(red: 255, green: 0, blue: 0, alpha: 0).cgColor // Red colour as a default
            }
        ...
        checkPermissionsThenExecute(permissionsGrantedAction: {
            ...
            for ekCalendar in ekCalendars {
                let calendar = Calendar(
                    ...
                    color: (NSColor(cgColor: ekCalendar.cgColor) ?? NSColor.cyan).rgb()!,
