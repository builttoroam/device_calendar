...
- Added `createOrUpdateEvents(List<Event>)` to save multiple events in one call, validating every event up front so a bad event later in the list can't leave earlier ones half-saved (fails with the 0-based index of the first invalid event). Implemented as `events.length` sequential calls to the existing single-event native path rather than a new native batch method -- see the method's doc comment for why (avoids duplicating this plugin's large per-event native logic into a second, harder-to-verify `ContentResolver.applyBatch`/`EKEventStore` deferred-commit code path for a round-trip-count optimization most callers won't need).
- Added `retrieveFreeBusy(calendarIds, start, end)` and a new `FreeBusyPeriod` model, computing merged busy/tentative/unavailable periods across one or more calendars. Implemented entirely in Dart over the existing `retrieveEvents` (dropping `Availability.Free` events, merging overlapping/touching periods, keeping the most restrictive status on overlap) since neither Android's `CalendarContract` nor iOS's `EventKit` expose a free/busy primitive -- no new native code or channel plumbing needed.
- Added iCalendar (RFC 5545, `.ics`) export/import: `Event.toIcs()`, `eventsToIcs(List<Event>)`, and `eventsFromIcs(String)`. Pure Dart, no native/channel involvement (neither platform has an OS API for this either). Hand-rolled rather than a pub.dev dependency, scoped to what `Event` actually needs rather than being a general-purpose RFC 5545 library -- see the doc comment on `eventsToIcs` for the specific simplifications this makes (dates round-trip correctly as instants via UTC/known-`TZID` resolution, but there's no `VTIMEZONE` component reader/writer; `VALARM` blocks are skipped on import; attendee `PARTSTAT` isn't read/written, since that's covered by the new `updateAttendeeStatus`; the iCalendar `UID` is treated as belonging to the `.ics` file, not this plugin's device-assigned `Event.eventId`, so import never sets it).
